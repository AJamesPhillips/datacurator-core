...
export function parse_specialised_objects_fromto_server (data: SpecialisedObjectsFromToServer | null)
{
    ...
    if (data)
    {
        type OldSpecialisedObjects = SpecialisedObjectsFromToServer & {
            wcomponent_ids_to_delete?: string[]
        }
        delete (data as OldSpecialisedObjects).wcomponent_ids_to_delete  // Can remove this once all user's data is cleared of this attribute
