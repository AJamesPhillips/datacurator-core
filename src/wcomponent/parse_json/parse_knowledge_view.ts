...
export function parse_knowledge_view (knowledge_view: KnowledgeView, wcomponent_ids?: Set<string>, remove_passthrough_entries = false): KnowledgeView
{
    ...
    knowledge_view = {
        ...
        wc_id_map,
        // TODO document why sort_type might be undefined
...
{
    type OlderKnowledgeViewWComponentEntry = KnowledgeViewWComponentEntry & { deleted?: true }

    const { wc_id_map } = knowledge_view
    const new_wc_id_map = { ...wc_id_map }
    Object.values(new_wc_id_map).forEach(entry =>
    {
        if ((entry as OlderKnowledgeViewWComponentEntry).deleted)
        {
            delete (entry as OlderKnowledgeViewWComponentEntry).deleted
            entry.blocked = true
        }
    })
